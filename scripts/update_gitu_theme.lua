-- re writes the gitu toml file so that it matches the theme from themes.toml
...
local function generate_toml(data)
	...
	toml = toml .. "[style]\n"
	toml = toml ..
	    string.format('section_header = { fg = "%s", mods = "%s" }\n', data.style.section_header.fg,
		    data.style.section_header.mods)
	toml = toml .. string.format('file_header = { fg = "%s" }\n', data.style.file_header.fg)
	toml = toml ..
	    string.format('syntax_highlight.keyword = { fg = "%s" }\n', data.style.syntax_highlight.keyword.fg)
	toml = toml .. string.format('hash = { fg = "%s" }\n', data.style.hash.fg)
	toml = toml .. string.format('tag = { fg = "%s" }\n', data.style.tag.fg)
	toml = toml ..
	    string.format('cursor = { symbol = "%s", fg = "%s" }\n', data.style.cursor.symbol, data.style.cursor.fg)
	toml = toml ..
	    string.format('selection_bar = { symbol = "%s", fg = "%s", mods = "%s" }\n', data.style.selection_bar.symbol,
		    data.style.selection_bar.fg, data.style.selection_bar.mods)
