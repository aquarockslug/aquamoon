...
local CORE = {
	...
	"eza", "dust", "duf", "htop", "mpv", "imv", "lazygit", "chafa", "neovide", "firefox",
	"networkmanager", "gammastep", "playerctl", "bluetui", "polkit-gnome", "libnotify", "dbus",
}

local AUR = { "clapboard-git", "hilbish-git", "networkmanager_dmenu", "gtklock-modules",
	"swaylock-effects", "river-vector-git" }


local EXTRA = {
	"godot", "gimp", "qutebrowser", "steam", "discord",
...
local function install(label, pkgs, pm)
	...
	if #need == 0 then
		print("  " .. label .. ": already installed"); return
	end
