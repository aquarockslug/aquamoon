...
hilbifetch.addInfo('shell', function()
	...
	local _, out = hilbish.run(shellbin .. ' --version | head -n 1', false)
...
hilbifetch.addInfo('terminal', function()
	local termbin = os.getenv 'TERM'
	local _, out = hilbish.run(termbin .. ' --version | head -n 1', false)
